...
csl: /home/luke/Documents/bibliography/styles/vancouver-brackets-no-et-al.csl
...
fig_gee <- fig('gee', 'Longitudinal associations of individual non-esterified fatty acids (mol% and nmol/mL) with insulin sensitivity and beta-cell function over the 6 years in the PROMISE cohort.  Generalized estimating equation models were adjusted for time, sex, ethnicity, baseline age, WC,
ALT, and family history of diabetes. Outcome variables were log-transformed, predictor variables were scaled, and x-axis values were exponentiated to represent percent difference per SD increase in the fatty acid. P-values were adjusted for the BH false discovery rate, presented as the dot size.')
...
20:3n-6. Raw values from the GEE models are shown in `r cite_st('gee')` for
...
low group) was `r misclass.n.percent.bcf$Percent[9]` in the low group,
`r misclass.n.percent.bcf$Percent[5]` in the middle group, and 
`r misclass.n.percent.bcf$Percent[1]` in the high group (the distribution of the
groupings based on the components from PLS-DA are shown in `r cite_sf('plsda_groups')`). 
The loadings plot of the NEFA composition against the extracted PLS-DA
components is shown in `r cite_sf('plsda_loadings')` and is used to identify
...
`r misclass.n.percent.dysgly$Percent[1]`
of participants were correctly predicted to not have dysglycemia, only 
`r misclass.n.percent.dysgly$Percent[4]` were correctly predicted to have
dysglycemia (see `r cite_sf('plsda_groups_dysgly')` for the distribution of 
dysglycemia status based on the components from the PLS-DA and 
`r cite_sf('plsda_loadings_dysgly')` for the loadings plot of the NEFA composition
...
`r cite_sf('plsda_groups')` and `r cite_sf('plsda_groups_dysgly')` show the
...
predictor result plots as seen in `r cite_sf('plsda_loadings')` and
`r cite_sf('plsda_loadings_dysgly')` show which of the predictor variables
