...
```{r ESM_Fig2, fig.cap=sfig_corr}
plot_heatmap(ds)
```

```{r ESM_Fig3, fig.cap=sfig_gee}
...
```{r ESM_Fig4, fig.cap=sfig_lcmm}
plot_lcmm_results(lcmm_data)
```

```{r ESM_Fig5, fig.cap=sfig_plsda_groups}
plot_plsda_grouping(plsda_results, 'Beta-cell function')
```

```{r ESM_Fig6, fig.cap=sfig_plsda_loadings, fig.width=7, fig.height=7}
plot_plsda_loadings(plsda_results)
```

```{r ESM_Fig7, fig.cap=sfig_plsda_groups_dysgly}
plot_plsda_grouping(plsda_dysgly, 'Dysglycemia')
```

```{r ESM_Fig8, fig.cap=sfig_plsda_loadings_dysgly, fig.width=7, fig.height=7}
plot_plsda_loadings(plsda_dysgly)
```

```{r ESM_Fig9, fig.cap=sfig_plsda_example}
