...
`r female$npct` females and `r european$npct` had European-ancestry (see 
`r cite_t("basic")`). The mean follow-up time was `r calc_followup_time()`
years, where `r calc_pct_full_visits()`% of participants attended all three
visits.
...
participants, `r famhist$npct`, had a family history of diabetes.
Insulin sensitivity and beta-cell function measures showed a significant median
decline from baseline to 6-years between
`r chg_outcomes$chg` (p`r chg_outcomes$p`, based on a time-adjusted GEE model)
in this analysis.
Consistent with this decline, there were `r calc_incid_dysgly("ConvertDM")`
and `r calc_incid_dysgly("ConvertPreDM")`incident cases of diabetes and pre-diabetes (IFG and IGT),
respectively, over the 6-years. Incident diabetes cases were excluded from further GEE
analyses.
...
were `r nefa$c[1]`, `r nefa$c[2]`, `r nefa$c[3]`, and 
`r nefa$c[4]`. Raw concentration values are shown in `r cite_st('nefa')`. 
There were no differences in individual NEFA for those who did compared to those
who did not take medications to manage blood lipids (data not shown).
All individual NEFA as well as the total fraction had
correlations that ranged from weak to null (r<0.3) with participant
characteristics (see `r cite_sf('corr_conc')` and `r cite_sf("corr_pct")`).
...
While full model adjustment (`r cite_f('gee_adj')`) attenuated most of these
...
`r est_gee$est[1]`% lower IGI/IR and `r est_gee$est[2]`% lower ISSI-2 at each
...
ISSI-2 (data not shown), though this may be due to the fact that only a subset
of participants at the 6 year visit had TAG measured. Sensitivity analyses to
examine the association with
...
and the individual NEFA on any of the outcome measures.
Raw values from the GEE models are shown in `r cite_st('gee_unadj')` for
unadjusted models and `r cite_st('gee_adj')` for fully adjusted models.
Consistent with the GEE modeling, results from modeling all NEFA in a
multivariate environment using PLS showed no discernible clustering nor was it
predictive of the outcomes (data not shown).
