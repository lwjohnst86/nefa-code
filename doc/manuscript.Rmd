...
```{r figure1, fig.cap=fig_dist}
plot_nefa_distribution(ds)
```

```{r figure2, dependson='geeData', fig.cap=fig_gee}
...
poor discriminatory ability of the PLS-DA model, with `r cite_sf('plsda_example')` 
...
```{r onlineSupplementalTableS2}
table_distribution(ds, stab_dist)
```

```{r onlineSupplementalFigureS2, fig.cap=sfig_corr}
plot_heatmap(ds)
```

```{r onlineSupplementalFigureS3, fig.cap=sfig_gee}
plot_gee_results(gee_results_unadj)
```

```{r onlineSupplementalTableS3_4}
table_gee(gee_results, stab_gee)
table_gee(gee_results_unadj, stab_gee_unadj)
```

```{r onlineSupplementalFigureS4, fig.cap=sfig_lcmm}
plot_lcmm_results(lcmm_data)
```

```{r onlineSupplementalFigureS5, fig.cap=sfig_plsda_groups}
plot_plsda_grouping(plsda_results, 'Beta-cell function')
```

```{r onlineSupplementalFigureS6, fig.cap=sfig_plsda_loadings, fig.width=7, fig.height=7}
plot_plsda_loadings(plsda_results)
```

```{r onlineSupplementalFigureS7, fig.cap=sfig_plsda_groups_dysgly}
plot_plsda_grouping(plsda_dysgly, 'Dysglycemia')
```

```{r onlineSupplementalFigureS8, fig.cap=sfig_plsda_loadings_dysgly, fig.width=7, fig.height=7}
plot_plsda_loadings(plsda_dysgly)
```

```{r onlineSupplementalFigureS9, fig.cap=sfig_plsda_example}
