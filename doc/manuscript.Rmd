...
[@Greenland1999a] recommendations (see `r cite_sf("dag")`), and quasi-likelihood information criterion 
(QIC). `r cite_st('qic')` shows the covariates compared using QIC. To reduce
complexity between models using either insulin sensitivity or beta-cell function
measures, only the model that had similar good fit for both was used. 
As such, we selected the model
that had the fewest covariates and that had similar fit between the outcome
measures (M9). The final GEE model was adjusted for years from baseline (time),
sex (female or male), ethnicity (European ancestry or non-European ancestry),
baseline age, WC, ALT, and MET (physical activity). Sex, ethnicity, and baseline 
age were time-*independent* variables. After scaling,
...
partial least squares (PLS) was used with the outcomes as the independent variable.
