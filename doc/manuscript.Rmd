...
TODO:
- Adjust for baseline prediabetes? SenAnalysis?
- Do PLS-LDA instead of LDA, since it is apparently better for this type of
analysis/data
...
fig_lda <- fig('lda', 'Linear discriminant analysis to identify patterns of NEFA composition between tertiles of log(ISSI-2) in 463 PROMISE participants at the baseline visit (2004-2006).')
...
sfig_lda <- sfig('lda', 'Linear discriminant factors with explained variance against tertiles of log(ISSI-2) in 463 PROMISE participants at the baseline visit (2004-2006).')
```
...
research objective was to examine the longitudinal role of the NEFA composition
on insulin sensitivity and beta-cell function. Some specific hypotheses included
that higher palmitic acid while lower polyunsaturated fatty acids such as
eicosapentaenoic acid would associate with lower insulin sensitivity and
beta-cell function.
...
analysis, linear discriminant analysis (LDA) was used to identify the patterns
of NEFA composition for tertiles of the metabolic disorders. Since LDA can't
handle longitudinal data, only the baseline data was used and only the outcome
and predictor variables could be included in the analysis. Data from the later
visit dates were used to confirm the results of the LDA analysis.

All analyses were performed using R `r getRversion()` [@Rbase], along with the R
packages geepack `r packageVersion('geepack')` for GEE [@Hoejsgaard2006a] and 
MASS `r packageVersion('MASS')` for LDA. The R code for this manuscript is
available at {{link}}. Results were statistically significance at p<0.05, after
adjusting for multiple testing using the False Discovery Rate [@Benjamini1995a].
...
gee_results %>% str
# Use substr after this above to make the Xterms go in order.
...
lda_df <- get_lda_data(ds)
lda_results <- lda(BCF ~ ., data = lda_df)
```

```{r inlineResults}
pctne <- calculate_percent_nefa_contribution(ds) 
misclass_npct <- calculate_misclass_npct(lda_results, lda_df)
pctChg.o <- calculate_outcomes_pct_change(ds)

```

## Basic characteristics of the PROMISE cohort

The primary outcome variables declined between `r pctChg.o` over the 6 years.
...
NEFA fraction. The largest contributor was `r pctne$c[1]`, followed by
`r pctne$c[2]`, `r pctne$c[3]`, and `r pctne$c[4]`.

All individual NEFA as well as the total fraction had weak to no correlations
(r<0.3) with the basic characteristics (see `r cite_sf('corr')`).

## GEE modeling

Several associations were seen in the unadjusted GEE models, particularly for 
the concentration data. Full model adjustment attenuated most of these 
associations, with total NEFA, 16:0, and 18:2n-6 (all as nmol/mL) had negative 
associations between IGI/IR and ISSI-2 (see `r cite_f('gee')`). The magnitude of
association for each of these variables was fairly consistent for each beta-cell
function measure. For every one SD decrease in any of these three NEFA 
variables, there was a predicted {{magnitude}}% lower IGI/IR and a 
{{magnitude}}% lower ISSI-2 at each clinic visit. None of the NEFA variables as 
a mol% had any association with the outcomes. Adjusting for TG attenuates all 
associations with IGI/IR and ISSI-2 (data not shown), though sample size at the 
6 year visit is restricted to n=128 in those who had TG measured. There were no
significant interaction effects between time and the individual NEFA on any of
the outcome measures (data not shown).

## Discriminant analysis

There was poor discriminatory ability in identifying patterns between tertiles 
of ISSI-2, with only `r misclass_npct` of participants in the baseline visit who
were classified correctly. In general, participants in the 3rd tertile (a high
ISSI-2) had less of all NEFA, though the mean differences between tertiles were
minor (`r cite_f('lda')`). The separation of tertiles of log(ISSI-2) by the two
linear discriminants is shown in `r cite_sf('lda')`. Analyses on the later
visits to confirm the baseline results were not substantially different from the
baseline visit.
...
[@Gray2013a;@Magkos2012a;@Kehlenbrink2012a], suggesting that either NEFA only 
have a minor role in IR or that NEFA is the effect, but not cause, of the 
underlying IR. Adipose tissue, which release NEFA during fasting, can become 
...
This is the first study to our knowledge to examine the composition of the NEFA fraction and 
...
Composition of NEFA and diabetes status (x-sec) [@Liu2010a], found 16:0, 18:0,
18:1, 18:2, 18:3 could be useful biomarkers for identifying healthy vs diabetes
...
poor, there was a general pattern that most NEFA were lower in individuals with
higher beta-cell function. However, the differences between groups were minor
and confirms the findings from the GEE modeling.

These results confirm the findings from the longitudinal GEE modeling of
individual NEFA variables.
...
gee_results[sort(order(
    gee_results$Xterms)[
        gee_df %>%
    select(matches('ne\\d\\d')) %>%
    gather(FA, Conc) %>%
    transmute(FA = renaming_fa(FA)) %>%
    unique() %>% 
        .[[1]]
    ])]
order(gee_results$Xterms)
plot_gee_results(gee_results)
gee_results %>% filter(Xterms == 'Total')
gee_results %>% str
```

```{r figLDA, fig.cap=fig_lda}
...
as including both original age and the time variable would result in
...
ds %>% 
    filter(VN == 0) %>% 
    design('cor', method = 'pearson') %>%
    lay_base(c(outcomes, 'BMI', 'Waist', 'Age', 'lALT', 'lTAG', 'Chol', 'HDL', 'LDL'), ne_conc) %>%
    build() %>%
    polish(rename.vars.fun = function(x) renaming_list(x) %>%
               gsub('^l(ALT|TAG)', 'log(\\1)', .)) %>% 
    trance('heatmap') %>% 
    visualize(colours = c('darkred', 'darkblue'), number.colours = 5) %>% 
    vision_simple() +
...
```{r supFigLDA}
visualize(lda_results, orig.data = lda_df, group.name = 'BCF') %>% 
    vision_simple()
```
