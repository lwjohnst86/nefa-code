...
- Key is that this is fasting, not postprandial!
...
sfig_consort <- sfig('consort', 'CONSORT diagram....')
...
sample size at each visit is shown in `r cite_sf('consort')`.  At each 
...
At each examination, participants had an 8-12 hour fasting blood sample 
taken followed by a 75g oral glucose tolerance test (OGTT) with a 30 minute and
...
Cholesterol, HDL, and triglycerides (TG) were measured using Roche Modular's
...
(QIC). `r cite_sf('qic')` shows the covariates compared using QIC and 
`r cite_sf('dag')` shows the specified directed acyclic graph. The final GEE model
(M#) we selected had time, sex, ethnicity, baseline age, waist circumference,
{{...}}, and family history of diabetes as covariates. <!--(exponentiated, minus
1, and multiplied by 100)--> After scaling, log-transforming, and exponentiating
the GEE estimates, the GEE results are interpreted as an expected percent
difference in the outcome variable for every SD increase in the predictor
variable given the covariates are held constant. An time by predictor term was
included in each of the GEE models to test for time interactions. Given that TG
is a risk factor for diabetes and that NEFA contribute to TG production, TG may
act as a mediator between NEFA and the outcomes. However, only a subset of the
participants at the 6-yr visit had clinically measured TG. To determine the role
of TG in the association between NEFA and the outcomes, TG was included in the
GEE model as a sensitivity analysis.
...
```{r inlineLDA}
fit.class <- predict(lda_results, newdata = lda_df[, -1])$class %>% 
    factor(., ordered = TRUE)

# Amount of participants who were misclassified
misclass <- table(fit.class, lda_df[[1]]) %>% 
    as.data.frame() %>% 
...
good_class <- table(fit.class, lda_df[[1]]) %>% 
    as.data.frame() %>% 
    filter(fit.class == Var2) %>% 
    summarize(count = sum(Freq)) %>% 
    as.numeric()

misclass_npct <- paste0(misclass, ' (', round(misclass / (good_class + misclass) * 100, 0), '%)')
```

```{r inlineMisc}
pctne <- calculate_percent_contribution(ds) 
...
individual NEFA made up the majority (`r sum(pct_nefa$pct)`%) of the total NEFA
fraction. The largest contributor was `r pctne$c[1]`, followed by `r
pctne$c[2]`, `r pctne$c[3]`, and `r pctne$c[4]`.
...
of ISSI-2, with only `r misclass_npct` of participants in the baseline
visit who were classified correctly. In general, participants in the 3rd tertile
(a high ISSI-2) had less of all NEFA, though the mean differences between
tertiles were minor (`r cite_f('lda')`). The separation of tertiles of
log(ISSI-2) and linear discriminants is shown in `r cite_sf('lda')`.

These results confirm the findings from the longitudinal GEE modeling of
individual NEFA variables.

Results between visits were not substantially different.
