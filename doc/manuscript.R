...
plsda_dysgly <- dysgly_data %>% 
    dplyr::mutate(ConvertDysgly = factor(ConvertDysgly, levels = c(0, 1), labels = c('No', 'Yes'))) %>% 
...
sample.outcomes <- calculate_sample(ds)
...
M2 <- stats::update(M0, . ~ . + Waist + BaseAge + Ethnicity + Sex + ALT) # best
M3 <- stats::update(M0, . ~ . + Waist + BaseAge + Ethnicity + Sex + ALT + MET) # best
M4 <- stats::update(M0, . ~ . + Waist + BaseAge + Ethnicity + Sex + ALT + MET + AlcoholPerWk) # best
M5 <- stats::update(M0, . ~ . + Waist + BaseAge + Ethnicity + Sex + ALT + MET + AlcoholPerWk + FamHistDiab) # best
M6 <- stats::update(M0, . ~ . + Waist + BaseAge + Ethnicity + Sex + ALT + MET + AlcoholPerWk + FamHistDiab + TobaccoUse) # best
M7 <- stats::update(M0, . ~ . + Waist + BaseAge + Ethnicity + Sex + ALT + FamHistDiab + TobaccoUse) # best
M8 <- stats::update(M0, . ~ . + Waist + BaseAge + Ethnicity + Sex + ALT + FamHistDiab) # best
M9 <- stats::update(M0, . ~ . + Waist + BaseAge + Ethnicity + Sex + ALT + FamHistDiab + TotalNE:VN)

qic_is_tab <- MuMIn::model.sel(M0, M1, M2, M3, M4, M5, M6, M7, M8, M9, rank = MuMIn::QIC) %>% 
...
M7 <- stats::update(M0, . ~ . + Waist + BaseAge + Ethnicity + Sex + ALT + FamHistDiab + TobaccoUse) # best
M8 <- stats::update(M0, . ~ . + Waist + BaseAge + Ethnicity + Sex + ALT + FamHistDiab) # best
M9 <- stats::update(M0, . ~ . + Waist + BaseAge + Ethnicity + Sex + ALT + FamHistDiab + TotalNE:VN) # best

qic_bcf_tab <- MuMIn::model.sel(M0, M1, M2, M3, M4, M5, M6, M7, M8, M9, rank = MuMIn::QIC) %>% 
    tibble::rownames_to_column() %>%
    dplyr::select(Model = rowname, QIC = IC, Delta = delta) %>% 
...
## ----ESM_Fig2, fig.cap=sfig_corr-----------------------------------------
plot_heatmap(ds)

## ----ESM_Fig3, fig.cap=sfig_gee------------------------------------------
...
## ----ESM_Fig4, fig.cap=sfig_lcmm-----------------------------------------
plot_lcmm_results(lcmm_data)

## ----ESM_Fig5, fig.cap=sfig_plsda_groups---------------------------------
plot_plsda_grouping(plsda_results, 'Beta-cell function')

## ----ESM_Fig6, fig.cap=sfig_plsda_loadings, fig.width=7, fig.height=7----
plot_plsda_loadings(plsda_results)

## ----ESM_Fig7, fig.cap=sfig_plsda_groups_dysgly--------------------------
plot_plsda_grouping(plsda_dysgly, 'Dysglycemia')

## ----ESM_Fig8, fig.cap=sfig_plsda_loadings_dysgly, fig.width=7, fig.height=7----
plot_plsda_loadings(plsda_dysgly)

## ----ESM_Fig9, fig.cap=sfig_plsda_example--------------------------------
