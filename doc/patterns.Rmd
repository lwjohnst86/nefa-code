...
dm.convert <- 
    table(ds.dysgly$ConvertDM)[2] %>% 
    {paste0(., ' (', round((. / 477)*100, 0), '%)')}
predm.convert <- 
    table(ds.dysgly$ConvertPreDM)[2] %>% 
    {paste0(., ' (', round((. / 477)*100, 0), '%)')}
```
...
ds.lcmm2 <- 
    full_join(ds.lcmm,
              ...
              by = 'SID') %>%
    mutate(class = factor(class, levels = c('High'), labels = c('Mid', 'High', 'Low')))
...
calculate_plsda_misclass(plsda_dysgly)
```
